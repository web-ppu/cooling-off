 ...
 * 가격 → 냉각 기간 로직.
 *
 * ⚠️ 이 파일은 web의 `src/lib/cooling.ts` 본문을 **복사**한 것이다 (tech-spec §2).
 * web↔확장이 같은 정책을 공유하기 위함이며, 드리프트는 `tests/cooling.sync.test.ts`
 * 스냅샷 테스트가 CI에서 잡는다. web 쪽을 수정하면 이 파일도 같이 수정해야 한다.
 */
