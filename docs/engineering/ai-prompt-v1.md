...
supersedes: v0.2 ([END] 토큰 기반 종료 규칙)
---

# AI Chat Prompt — v0.3

> 현재 Decide 플로우 기준 프롬프트. AI 응답 구조는 [`./ai-frontend-contract.md`](./ai-frontend-contract.md)를 따른다.
>
> **관련 문서**
...
| 산수 | 사용 빈도·가격·유지비를 계산해 체감 비용으로 바꿈 | "월 4번이면 한 번에 2만원대야." |

## 6. 대화 단계

AI 응답은 항상 `phase`를 포함한다.

```json
{
  "message": "사용자에게 보여줄 AI 메시지",
...
| phase | 의미 | 버튼 |
|-------|------|------|
| `gathering` | 정보 수집, 구체화 질문 | [결정하기] 미노출 |
| `perspective` | 산수·대안·모순 등 관점 제시 | [결정하기] 노출 |
| `closing` | 대화 마무리 제안 또는 최대 턴 도달 | [결정하기] 노출 |

[결정하기] 버튼은 한 번 노출되면 사라지지 않는다.

## 7. 팩트 요약

`summary`는 `closing` 단계에서만 제공한다.
대화에서 나온 사실만 bullet으로 정리한다.
판단, 조언, 결론은 넣지 않는다.
...
요약할 사실이 1개도 없으면 `summary`는 `null`로 둔다.
...
- 최대 10턴.
- 10턴째 응답에서는 수집된 정보 안에서 가능한 관점을 제시하고 `closing`으로 마무리한다.
...
- summary에는 현재 대화에서 나온 사실만 포함한다.

## 10. 출력 규칙

- 응답은 1~3문장.
- 사용자가 답하기 쉬운 구체 질문을 우선한다.
- JSON 구조는 프론트엔드 계약과 맞춘다.
- `[END]` 토큰은 사용하지 않는다.
...
| 시나리오 | 기대 동작 |
|----------|-----------|
| 사용자가 추상적으로 답함 | 구체화 질문·산수만 사용 |
| 등록 사유와 현재 주장이 달라짐 | 모순을 짚고 실제 빈도 질문 |
| 답변이 너무 짧음 | 원 질문을 구체화해서 다시 묻기 |
| 사용자가 충분히 구체적으로 답함 | 관점 제시 후 [결정하기]가 가능하도록 `perspective` 또는 `closing` |
