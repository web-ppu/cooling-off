...
1. **등록 시 적은 이유 인용** — 아래 `[등록 정보]` 섹션의 "사고 싶은 이유" 를 그대로 짚고, 그 중에 가장 끌리는 부분이 뭔지 묻는다. (v1.2: prompt caching #131 을 위해 본문 내 `{{purchaseReason}}` 변수 치환을 제거. 변수는 [등록 정보] 섹션 1곳에만 존재 → 가이드라인 본문이 모든 사용자에게 공통 prefix.)
