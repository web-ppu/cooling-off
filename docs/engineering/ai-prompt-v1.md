...
## 5-1-1. 무의미·해독 불가 입력 처리 — v1.3 신규 (issue #203)

QA: 의미 없는 입력("aaaa", "fdffdd", "adffd" 등)에도 AI 가 정상 답변을 반환 → 사용자가 "mock 데이터 아니냐"고 오인. 실제로는 실시간 Gemini 가 무의미 문자열을 억지로 해석해 답하던 것.

§5-1 의 **본론 이탈 처리**(의미는 있으나 빗나간 답)와는 다르다. 여기서 다루는 것은 **의미 자체를 해독할 수 없는 문자열**이다.

### Best practice — 다층(multi-layer) 가드레일

시스템 프롬프트만으로는 하드 가드레일이 아니다(모델이 중요한 순간 무시 가능). 값싼 룰 기반 입력 필터 + 프롬프트를 함께 둔다. 단, **false-positive(정상 단답 차단)** 가 가장 큰 리스크이므로 필터는 보수적으로.

- 참고: Kalvium Labs "Guardrails for LLM Applications", Datadog "LLM guardrails best practices", WorkBot "Stop Random AI Chatbot Answers".

### ① 클라이언트 1차 필터 (`ChatScreen.tsx` `isObviousGibberish`)

- **단일 문자 4회 이상 반복만** API 호출 전 차단하고 재입력 안내("무슨 말인지 잘 모르겠어. 왜 사고 싶은지 다시 적어줄래?"). 예) `aaaa`, `ㅋㅋㅋㅋ`, `....`
- `fdffdd` 같은 무작위 문자열·자음 난타(`ㅁㄴㅇㄹ`)는 **막지 않고 프롬프트에 위임** (false-positive 회피).
- `응`/`몰라`/`ㅁㄹ`/`ㄴㄴ` 등 짧지만 의미 있는 답은 **절대 차단하지 않는다**.

### ② 시스템 프롬프트 규칙 (`[무의미·해독 불가 입력 처리]`)

- 무의미 문자열을 사고 싶은 이유/대화로 해석·추측하지 않는다.
- 새 관점(#2~#6) 제시 금지. 메타 태그는 반드시 `[show_decide_button: false]` (무의미 입력으로 결정 단계 진입 차단).
- 짧게 되물어 재입력 유도. 반복돼도 도덕 평가·짜증·재촉 없이 차분히, 같은 문장 그대로 반복은 피한다.
- 의미 있는 한국어 단답은 §5-1 [본론 이탈 처리] 로 넘긴다.
