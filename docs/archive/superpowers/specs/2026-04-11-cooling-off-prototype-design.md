...
2. **결정 후 토스트**: "기록됐어요" + [실행 취소] (3초). Sonner `toast()` 사용.
...
- 선택 후:
  ...
  - 중립 토스트 "기록됐어요" + [실행 취소] 3초 (Sonner)
  - 실행 취소 클릭 시 status/decided_at 원상복구
  - 3초 후 `/` 이동 (undo 버튼 사라진 뒤)
...
1. **US-5 "3초 Undo 토스트"의 생명 주기**
   - 결정 후 메인으로 리다이렉트되는데 토스트가 어디서 떠 있어야 하나?
   - 메인까지 따라가야 하나? 결정 화면에 3초 머무르나?
   - 의심: PRD에 명시 없음. UX 검증 필요.
