...
상품 등록 시 사진을 넣을지 말지 고민이 있었다. 직접 업로드, OG 태그 자동 추출 등 방법을 검토했으나, **방법이 아니라 "이미지가 이 앱에 필요한가" 자체가 핵심 질문**이다.
...
쇼핑앱은 물건을 매력적으로 보여줘서 사게 만든다. 쿨링오프는 반대다. 앱의 목적은 wanting을 식히고 객관적으로 판단하게 하는 것인데, 상품 이미지는 wanting을 가장 강하게 자극하는 요소다.

### 3-2. 냉각기 중 정보 숨김 원칙과 충돌

PRD 원칙 7번: "냉각기 중에는 상품명, 가격, URL 전부 비노출 — 타이머만 보여줌." 이유는 다시 보면 wanting이 되살아나기 때문이다.
...
어느 쪽이든 이미지는 앱의 핵심 메커니즘(냉각)을 약화시킨다.

### 3-3. AI 대화에 이미지가 필요 없다

AI 대화 시뮬레이션 2개(에어팟 프로3, 러닝화)를 돌렸다. 둘 다 텍스트(상품명 + 가격 + 등록 사유)만으로:
- 정보 수집 → 관점 제시 → 자각까지 전체 대화가 성립
- AI가 "35만원어치 귀찮음" 산수, "배터리 교체 7~8만원" 대안 제시 등 모두 텍스트 기반

이미지가 없어서 대화가 약해지는 경우는 없었다. 상세: `../engineering/ai-simulation-log.md`

### 3-4. 30초 등록 원칙 위반 (직접 업로드의 경우)

PRD 원칙 6번: "30초 등록." 갤러리에서 사진을 찾아 선택하는 데 15~20초가 추가된다. 등록 마찰이 높아지면 "귀찮아서 안 쓰게" 되는 것이 가장 큰 리스크(A3 리스크)인데, 사진이 그걸 악화시킨다.
...
| 대안 | 기각 이유 |
|------|----------|
| 직접 업로드 | 30초 원칙 위반 + localStorage에 이미지 저장 복잡도 (Base64 or Supabase Storage 필요) |
...
이 결정을 뒤집을 수 있는 근거:
- 사용자 테스트에서 "뭘 등록했는지 기억이 안 나서 앱을 안 쓰게 됐다"는 피드백이 반복되는 경우
