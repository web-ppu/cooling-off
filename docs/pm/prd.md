...
6. **냉각기 중 자극 최소화**: 냉각 중에는 항목 구분에 필요한 이름과 남은 시간만 보여주고, 구매 욕구를 다시 키울 수 있는 상세 정보는 숨깁니다.
...
- 이름과 가격이 유효할 때만 [봉인하기]를 누를 수 있습니다.
- URL 오류는 등록을 막지 않습니다. 다만 링크로 열 수 있는 값으로 취급하지 않습니다.
- 등록이 완료되면 봉인 애니메이션 후 홈으로 이동합니다.
...
- 냉각 중에는 항목 이름과 남은 시간만 표시합니다.
- 냉각 중에는 가격, URL, 사고 싶은 이유, 이미지를 표시하지 않습니다.
- 냉각 중 항목은 사용자가 삭제할 수 있습니다.
- 삭제된 냉각 중 항목은 홈과 기록에 남기지 않습니다.
...
| 계정 | 계정 설정, 탈퇴, 데이터 삭제 |
| 진입점 | 공유시트, 북마클릿, OCR |
| 자동화 | URL 기반 상품명/가격 자동 입력 |
| 소셜 | 친구 공유, 결정 기록 공유, 익명 카운터 |
| 접근성 | 키보드 탐색, 화면 읽기, 모션 줄이기 |
