...
등록 화면 안에서는 알림 권한을 묻지 않는다. 알림 제안은 계정의 첫 등록이 끝난 뒤 홈에서 1회성 카드로 보여준다. 두 번째 이후 등록에서는 자동으로 다시 묻지 않는다.
...
- 첫 AI 메시지는 항상 `"네 말 들어볼게. 지금 이거 왜 사고 싶어?"`로 고정.
...
냉각기가 끝난 항목에 대해 알림을 제공한다. 트리거, 재노출 조건, 권한 처리, 플랫폼별 정책은 [`../pm/notification-policy.md`](../pm/notification-policy.md)에서 관리한다.

이 문서에서는 사용자가 보는 화면 배치만 정의한다.

### 4-1. 첫 냉각 시작 직후 알림 카드

첫 등록 완료 직후 홈의 문서 헤더 아래, 결정 대기/냉각 중 목록 위에 알림 카드를 배치한다.

```
┌──────────────────────────────────────┐
│  홈 헤더                              │
├──────────────────────────────────────┤
│  냉각이 끝나면 알려드릴까요?          │  ← 첫 등록 완료 직후 1회성 카드
│  5월 29일 14:00부터 다시 결정할 수... │
│  [푸시로 받기] [괜찮아요]             │
├──────────────────────────────────────┤
│  결정 대기 섹션                       │
│  냉각 중 섹션                         │
...
| 상황 | UI |
|------|----|
| Android / desktop / 이미 설치된 PWA | `냉각이 끝나면 알려드릴까요?` + [푸시로 받기] [괜찮아요] |
| iOS Safari 미설치 | `iPhone에서는 홈 화면에 추가하면 알림을 받을 수 있어요` + [홈 화면에 추가하고 알림 받기] [그냥 홈에서 확인할게요] |

- 방금 등록한 물건의 결정 가능 시점을 함께 표시한다.
- 가격, URL, 사고 싶은 이유, 상품 이미지는 다시 강조하지 않는다.
- 권한 요청 카드와 iOS 설치 안내는 About, 기록, AI 채팅 화면에는 넣지 않는다.
- 거부 버튼은 `괜찮아요`로 쓴다.

### 4-2. iOS 설치 안내 바텀시트

iOS Safari 미설치 사용자가 [홈 화면에 추가하고 알림 받기]를 선택하면 홈 위에 바텀시트를 연다.
...
- 이 단계에서는 푸시 권한 팝업을 띄우지 않는다.
- 사용자가 닫아도 냉각 중 항목은 홈에서 계속 확인할 수 있다.

### 4-3. 냉각 만료 알림

| 상황 | 제목 | 내용 | 선택 시 이동 |
|------|------|------|--------------|
| 냉각기 만료 | 결정할 시간입니다 | 냉각 시간이 끝났습니다. 준비되면 확인해보세요. | 홈의 결정 대기 섹션 |
| 여러 항목 만료 | 결정할 항목이 기다리고 있어요 | 냉각 시간이 끝난 항목이 있습니다. 준비되면 확인해보세요. | 홈의 결정 대기 섹션 |
...
- 알림을 허용하지 않은 사용자에게 "알려드릴게요"라고 단정하지 않는다.
...
### v1.5 (2026-05-27, 알림 안내 화면 반영)

| 변경 | 왜 바꿨는지 |
|------|-------------|
| 첫 등록 완료 직후 홈 상단에 알림 권한 카드를 배치 | 알림 제안이 첫 방문, About, 일반 홈 진입에 섞이지 않게 하기 위해 |
| iOS Safari 미설치 사용자의 홈 화면 추가 안내를 바텀시트로 정의 | iOS 푸시 제약을 사용자가 따라 할 수 있는 화면 흐름으로 풀기 위해 |
| 알림 미허용 사용자에게 "알려드릴게요"라고 단정하지 않는 기준 추가 | 실제 권한 상태와 화면 문구가 충돌하지 않게 하기 위해 |
| 알림 카드는 계정 기준 최초 1회만 노출하도록 명시 | 물건을 새로 등록할 때마다 권한 요청이 반복되는 오해를 막기 위해 |
