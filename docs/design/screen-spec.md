...
| 변경 | 왜 바꿨는지 |
|------|-------------|
| 냉각 중 화면에서 가격·URL·사고 싶은 이유 비노출로 수정 | 구매 욕구 재자극을 줄이는 PRD 원칙과 FR-3을 맞추기 위해 |
...
아이템 상태는 냉각 중, 결정 대기, 삼, 안 삼, 삭제됨 중 하나다.
AI 대화, 결정하기 버튼 노출, 팩트 요약은 아이템 상태가 아니라 Decide 화면 안에서만 쓰는 진행 단계다.
...
| 단계 | 다음 단계 | 계기 | 화면 처리 |
|------|-----------|------|-----------|
| AI 대화 | 결정하기 가능 | AI가 수집한 정보를 가공해 관점 제시 | [결정하기] 버튼 노출 |
| 결정하기 가능 | 팩트 요약 | [결정하기] 탭 / 최대 10턴 도달 | 채팅 입력창 비활성화 |
| 팩트 요약 | 결정 선택 | 팩트 요약 표시 완료 | [안 삼]/[삼] 표시 |

**핵심 결정:**

- 냉각 중 상태에서는 항목 이름과 남은 시간만 표시. 가격, URL, 사고 싶은 이유, 이미지는 비노출.
...
- 냉각 중 카드에는 항목 이름과 남은 시간만 표시한다. 가격, URL, 사고 싶은 이유, 이미지는 표시하지 않는다.
...
항목 이름과 남은 시간만 표시한다. 가격, URL, 사고 싶은 이유, 이미지는 표시하지 않는다. 타이머 만료 시 결정 대기 상태가 된다.
...
| 결정 버튼 노출 시점이 불명확함 | AI 응답은 왔지만 [결정하기] 버튼 노출 기준을 판단하기 어려우면 3턴 이후 [결정하기] 버튼 표시 |
