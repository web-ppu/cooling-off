...
-- 중복 등록 방지: 한 사용자가 결정 전(cooling/ready)인 동일 URL을 중복 등록할 수 없다.
-- 브라우저 확장의 중복 게이트(EX-6)와 web 등록 흐름의 공통 방어선.
-- decided(결정 완료)/soft-delete 항목은 제외 — 지난 구매를 다시 식힐 수 있어야 하므로
-- 동일 URL 재등록을 허용한다. url 이 null 이면(직접 입력 등록 등) 제약 대상이 아니다.
