...
export type ParseResult =
  ...
  | { ok: false; error: 'invalid_url' | 'blocked_host' | 'fetch_failed' | 'too_large' }
...
export async function parsePreview(rawUrl: string): Promise<ParseResult> {
  ...
  if (html === 'fetch_failed') return { ok: false, error: 'fetch_failed' }
  if (html === 'too_large') return { ok: false, error: 'too_large' }
...
function parseOg(html: string): OgResult {
  ...
  return {
    title: title ? decodeEntities(title).trim().slice(0, 80) || null : null,
